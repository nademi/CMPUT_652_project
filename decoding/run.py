...
## Tasks
# (1)- Reproducing the Honari result along with different word embedding models
# (2) - Adjective-Adjective display of subjects
# (3) - Resutl based on all subjects
import argparse
...
inData_opt = ['channels']
wordvec_opt = ['skipgram', 'gpt2', 'glove'] # gpt2 dimention 726, glove dimention 200, skip dimention 300
end_opt = ['local', 'cluster'] # should be deleted
avg_opt = ['random']
whcond_opt = ['straight',  # train on x, test on x
              ...
              'phrasal',
              'adjadj'] # This has been added by us (we used one of already written functions of honari) (Yousef and Mohammad)
...
if __name__ == '__main__':
    ...
    # notemp case
    elif args.notemp: # notempt: trains on the whole time after stumuli,def notemp=false
        time_start = 0 - eps
        time_end = 0.700
    ...
    if args.wvec == 'gpt2':
        gpt2 = sio.loadmat('{}/wordvecs/skipgram/gpt2_vecs.mat'.format(params.data_dir))
        params.word_dims = range(0, gpt2['vectors'].shape[1])
        params.word_vecs = gpt2['vectors']
        params.dist_metric = 'cosine'
        params.words = gpt2['words']

    if args.wvec == 'glove':
        glove = sio.loadmat('{}/wordvecs/skipgram/glov_vecs.mat'.format(params.data_dir))
        params.word_dims = range(0, glove['vectors'].shape[1])
        params.word_vecs = glove['vectors']
        params.dist_metric = 'cosine'
        params.words = glove['words']
    ...
    if args.isperm is False:
        ...
        if args.whcond == 'adjadj':
        # do_2v2_onesubj
        # all_2v2 = eval.do_2v2(range(2,4))
            all_2v2 = evaler.do_2v2_adj_adj_or_N_N(range(4, 5), range(0, 2))
